...
function NavbarComponent({ firmName, firmHotline, monFriTime }: { firmName: string; firmHotline: string; monFriTime: string }) {
  ...
  return (
    <header className="w-full fixed top-0 left-0 z-50">
      <div className="hidden md:flex w-full bg-neutral-950 text-neutral-400 text-sm font-sans uppercase tracking-widest py-4 px-8 lg:px-16 justify-between items-center border-b border-neutral-900">
        <div className="flex items-center gap-10">
          <span className="flex items-center gap-3"><Clock size={16} className="text-amber-500" /> Chambers: {monFriTime}</span>
          <span className="flex items-center gap-3"><Scale size={16} className="text-amber-500" /> Executive Intake Grid</span>
        </div>
        <div className="tracking-widest text-sm">Urgent Advisory: <span className="text-amber-500 font-bold text-base whitespace-nowrap">{firmHotline}</span></div>
      </div>
    ...
    <section className="relative w-full min-h-[90vh] flex items-center justify-center px-8 lg:px-16 overflow-hidden bg-neutral-950 py-24">
      ...
      <div className="max-w-7xl w-full mx-auto grid grid-cols-1 lg:grid-cols-12 gap-16 items-center z-10 relative py-16">
        <div className="lg:col-span-8 space-y-10">
          ...
          {/* Fixed line structure to prevent layout breaks on long text tracking rows */}
            ...
            <div key={index} className="bg-neutral-900/20 border-2 border-neutral-900 p-10 lg:p-12 flex flex-col justify-between relative hover:border-neutral-800 transition-all rounded-none shadow-2xl">
              ...
              <div className="space-y-6 relative z-10">
                <div className="flex items-center gap-1.5 text-amber-500">
                  {[...Array(typeof item.rating === "number" ? item.rating : 5)].map((_, i) => <Star key={i} size={18} fill="currentColor" />)}
                </div>
...
// ==========================================
function FooterComponent({ firmName, firmTagline, firmHotline, firmEmail, firmAddress, timings, social }: any) {
  const monFriTime = timings.monFri || "Mon - Fri: 9:00 AM - 6:00 PM";
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 lg:gap-16 pb-20 border-b-2 border-neutral-900">
          ...
          <div className="md:col-span-3 space-y-6 uppercase tracking-widest text-xs lg:text-sm">
            <h4 className="font-extrabold text-white tracking-widest border-b-2 border-neutral-900 pb-3 text-sm lg:text-base">Fields of Practice</h4>
            <ul className="space-y-4 text-neutral-500 font-semibold lowercase tracking-normal first-letter:uppercase text-base">
              <li><a href="#practices" className="hover:text-amber-500 transition-colors block py-0.5">Corporate Litigation</a></li>
              <li><a href="#practices" className="hover:text-amber-500 transition-colors block py-0.5">Banking Regulation</a></li>
